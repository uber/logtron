...
function createLogger(opts, cb) {
    ...
    socket.once('connect', function onSocketOpen() {
        socket.destroy();
        ...
        client.connect(function onConnect(err) {
            ...
            client.create_consumer(uuid(), 'rt-foobar', {},
                function onStream(stream) {
                    ...
                    logger.readStream = function readStream(listener) {
                        stream.once('data', function onData(chunk) {
    ...
    socket.once('error', function onError() {
        socket.destroy();
